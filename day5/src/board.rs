...
impl Board {
    pub(crate) fn marked_at_least(&self, p0: u16) -> usize {
        self.fields.iter()
...
impl Board {
    pub fn mark(&mut self, line: Line) {
        assert!(line.is_horizontal_or_vertical(), "Only horizontal and vertical are supported for now");
        line.point_vec().into_iter().for_each(|point|self.fields[[point.x as usize,point.y as usize]] += 1);
    }
    pub fn mark_part2(&mut self, line: Line) {
        assert!(line.is_horizontal_vertical_or_diagonal(), "Only horizontal and vertical are supported for now");
        line.point_vec().into_iter().for_each(|point|self.fields[[point.x as usize,point.y as usize]] += 1);
    }
...
mod board_tests {
    ...
    fn part2_mark_test() {
        let mut board = Board::with_dim(10);
        board.mark_part2("1,1 -> 3,3".parse().unwrap());
        board.mark_part2("9,7 -> 7,9".parse().unwrap());
        println!("{:?}",board);
        assert_eq!(board.fields[[1,1]],1);
